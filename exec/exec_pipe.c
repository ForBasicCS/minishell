...
/*   Updated: 2021/10/10 19:05:49 by hynam            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	here_doucument(t_cmd **cmd, int fd)
{
	char	*str;
...
int	exec_pipe(t_cmd **cmd)
{
	...
	if ((*cmd)->next->word[0] == NULL)
		return (0);
	if ((*cmd)->p_type == 0)
		return (pipe_redir(cmd));
	else
		return (inout_redir(cmd));
}
